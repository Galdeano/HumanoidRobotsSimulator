...
// [Refactored to ws]     static Struct_uLINK *ws->uLINKc;
// [Refactored to ws]     static gsl_matrix * ws->Inertia_Motor;
// [Refactored to ws]     static gsl_matrix *ws->A;
// [Refactored to ws]     static gsl_vector *ws->b;
// [Refactored to ws]     static gsl_vector *ws->tmp;
// [Refactored to ws]     static gsl_vector *ws->g;
// [Refactored to ws]     static gsl_vector *ws->ef;
// [Refactored to ws]     static gsl_vector *ws->stab;
// [Refactored to ws]     static gsl_vector *ws->u;

// [Refactored to ws]     static gsl_vector * ws->ddq;
// [Refactored to ws]     static gsl_permutation * ws->p;
// [Refactored to ws]     static gsl_matrix * ws->A2;
...
// [Refactored to ws]     static Struct_State ws->Statusc;
// [Refactored to ws]     static int ws->init=1;
    if (ws->init==1)
    {

// [Allocated in ws]         ws->uLINKc = calloc((Status->ddl)+2-6,sizeof(Struct_uLINK));
// [Allocated in ws]         ws->Inertia_Motor = gsl_matrix_calloc (nDoF, nDoF);
// [Allocated in ws]         ws->A = gsl_matrix_calloc (nDoF, nDoF);
// [Allocated in ws]         ws->b = gsl_vector_calloc (nDoF);
// [Allocated in ws]         ws->tmp = gsl_vector_calloc (nDoF);
// [Allocated in ws]         ws->g = gsl_vector_calloc (nDoF);
// [Allocated in ws]         ws->ef = gsl_vector_calloc (nDoF);
// [Allocated in ws]         ws->stab = gsl_vector_calloc (nDoF);
// [Allocated in ws]         ws->u = gsl_vector_calloc (nDoF);

// [Allocated in ws]         ws->ddq = gsl_vector_alloc (nDoF);
// [Allocated in ws]         ws->p = gsl_permutation_alloc (nDoF);
// [Allocated in ws]         ws->A2 = gsl_matrix_calloc (nDoF, nDoF);
...
//    ExternalForces
    ...
    // [Allocated in ws] gsl_vector * ws->f = gsl_vector_calloc (3);
    // [Allocated in ws] gsl_vector * ws->tau = gsl_vector_calloc (3);
...
//    }
    ...
    gsl_vector_free(ws->f);
    gsl_vector_free(ws->tau);
...
//      if ((t%50)==0)
    if ((t%(int)(Te/Dtime))==0)
    {
// [Refactored to ws]         static int ws->init_com=1;
// [Refactored to ws]         static double *ws->uPD;
// [Refactored to ws]         static double *ws->qd;
// [Refactored to ws]         static double *ws->dqd;
        if (ws->init_com==1)
        {
// [Allocated in ws]             ws->uPD = calloc((Status->ddl)-6,sizeof(double));
// [Allocated in ws]             ws->qd = calloc((Status->ddl)-6,sizeof(double));
// [Allocated in ws]             ws->dqd = calloc((Status->ddl)-6,sizeof(double));
            ws->init_com=0;
        }

// [Refactored to ws]         static double ws->com[3];
...
// [Refactored to ws]         static double ws->FootR[3]= {0.0853,0,-0.11};
...
// [Refactored to ws]         static double ws->FootL[3]= {0.0853,0,-0.11};
...
// [Refactored to ws]         static double *ws->uG;
// [Refactored to ws]         static double *ws->fG;
// [Refactored to ws]         static double *ws->tG;
// [Refactored to ws]         static double *ws->uStab;
// [Refactored to ws]         static int ws->init_G=1;
        if (ws->init_G==1)
        {
// [Allocated in ws]             ws->uG = calloc((Status->ddl)-6,sizeof(double));
// [Allocated in ws]             ws->fG = calloc((Status->ddl)-6,sizeof(double));
// [Allocated in ws]             ws->tG = calloc((Status->ddl)-6,sizeof(double));
// [Allocated in ws]             ws->uStab = calloc((Status->ddl)-6,sizeof(double));
            ws->init_G=0;
        }
...
// [Refactored to ws]         static double ws->uG[NbLinks-2], ws->fG[NbLinks-2], ws->tG[NbLinks-2];
        Gravity_f( ws->uLINKc, &ws->Statusc, 1, ws->fG, ws->tG);
        for (n=0; n<nDoF-6; n++)
        {
            ws->uG[n]=ws->uLINKc[n+2].ug;
        ...
        if (Visualisation)
        {
            ...
            for(n=0; n<nDoF-6; n++)
            {
                fprintf(q_file,"%ws->f ",ws->uLINKc[n+2].q);
            }
            fprintf(q_file,"\n");
            fclose(q_file);

            FILE *qd_file=fopen("./../Simu_data/ws->qd.txt","a");
            for(n=0; n<nDoF-6; n++)
            {
                fprintf(qd_file,"%ws->f ",ws->qd[n]);
            }
            fprintf(qd_file,"\n");
            fclose(qd_file);
            ...
            for(n=0; n<3; n++)
            {
                fprintf(pBody_file,"%ws->f ",gsl_vector_get(uLINK[1].p,n));
            }
            fprintf(pBody_file,"\n");
            fclose(pBody_file);
            ...
            {
                DoF-6;
                fprintf(ug1_file,"%ws->f ",ws->uLINKc[n+2].ug);
            }
            fprintf(ug1_file,"\n");
            fclose(ug1_file);
            ...
            for(n=0; n<nDoF-6; n++)
            {
                fprintf(ustab_file,"%ws->f ",ws->uStab[n]);
            }
            fprintf(ustab_file,"\n");
            fclose(ustab_file);
#endif
...
// [Refactored to ws]         static gsl_vector * ws->idx1;
// [Refactored to ws]         static gsl_vector * ws->idx2;
// [Refactored to ws]         static gsl_matrix * ws->J1;
// [Refactored to ws]         static gsl_matrix * ws->J2;
// [Refactored to ws]         static gsl_matrix * ws->JCoMR;
// [Refactored to ws]         static gsl_matrix * ws->JCoML;
// [Refactored to ws]         static gsl_matrix * ws->Jtilde;
// [Refactored to ws]         static gsl_vector * ws->task1;
// [Refactored to ws]         static gsl_vector * ws->task2;
// [Refactored to ws]         static gsl_vector * ws->taskCoMR;
// [Refactored to ws]         static gsl_vector * ws->taskCoML;
// [Refactored to ws]         static gsl_vector * ws->CoM;
// [Refactored to ws]         static gsl_vector * ws->vec3;
// [Refactored to ws]         static gsl_vector * ws->vec3_2;
// [Refactored to ws]         static gsl_vector * ws->p_task;
// [Refactored to ws]         static gsl_matrix * ws->R;
// [Refactored to ws]         static gsl_matrix * ws->P1;
// [Refactored to ws]         static gsl_matrix * ws->P2;
// [Refactored to ws]         static gsl_matrix * ws->PCoMR;
// [Refactored to ws]         static gsl_matrix * ws->PCoML;
// [Refactored to ws]         static gsl_matrix * ws->Ptilde;
// [Refactored to ws]         static gsl_matrix * ws->Ptmp;
// [Refactored to ws]         static gsl_matrix * ws->invJ;
// [Refactored to ws]         static gsl_matrix * ws->invJCoM;
// [Refactored to ws]         static gsl_vector * ws->dq;
// [Refactored to ws]         static gsl_vector * ws->dqtmp;
// [Refactored to ws]         static gsl_vector * ws->dqtmp2;
// [Refactored to ws]         static gsl_vector * ws->dq_old;
// [Refactored to ws]         static gsl_vector * ws->ddq_task;

// [Refactored to ws]         static double *ws->opd;
// [Refactored to ws]         static gsl_vector * ws->trace;

// [Refactored to ws]         static gsl_vector * ws->adphi;
// [Refactored to ws]         static double *ws->qdev;
// [Refactored to ws]         static gsl_vector * ws->CoP;
 //       static double ws->f=0.0;
// [Refactored to ws]         static gsl_vector * ws->zmp;
// [Refactored to ws]         static gsl_vector * ws->dzmp;

// [Refactored to ws]         static gsl_vector * ws->q_pd;

        static int path1[8] = {7, 7, 6, 5, 4, 3, 2, 1};
        static int path2[14] = {13, 13, 12, 11, 10, 9, 8, 2, 3, 4, 5, 6, 7, 7};

// [Refactored to ws]         static double *ws->Jf;
// [Refactored to ws]         static double *ws->invf;
// [Refactored to ws]         static double *ws->pf;
// [Refactored to ws]         static double *ws->Rf;
// [Refactored to ws]         static double *ws->taskf;
// [Refactored to ws]         static double *ws->dqf;

// [Refactored to ws]         static int ws->init_task=1;
        if (ws->init_task==1)
        {
// [Allocated in ws]             ws->idx1 = gsl_vector_calloc (8);
// [Allocated in ws]             ws->idx2 = gsl_vector_calloc (14);
// [Allocated in ws]             ws->J1 = gsl_matrix_calloc (6,nDoF-6);
// [Allocated in ws]             ws->J2 = gsl_matrix_calloc (6,nDoF-6);
// [Allocated in ws]             ws->JCoMR = gsl_matrix_calloc (3,nDoF-6);
// [Allocated in ws]             ws->JCoML = gsl_matrix_calloc (3,nDoF-6);
// [Allocated in ws]             ws->Jtilde = gsl_matrix_calloc (3,nDoF-6);
// [Allocated in ws]             ws->task1 = gsl_vector_calloc (6);
// [Allocated in ws]             ws->task2 = gsl_vector_calloc (6);
// [Allocated in ws]             ws->taskCoMR = gsl_vector_calloc (3);
// [Allocated in ws]             ws->taskCoML = gsl_vector_calloc (3);
// [Allocated in ws]             ws->CoM = gsl_vector_calloc (3);
// [Allocated in ws]             ws->vec3 = gsl_vector_calloc (3);
// [Allocated in ws]             ws->vec3_2 = gsl_vector_calloc (3);
// [Allocated in ws]             ws->p_task = gsl_vector_calloc (3);
// [Allocated in ws]             ws->R = gsl_matrix_calloc (3,3);
// [Allocated in ws]             ws->P1 = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->P2 = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->PCoMR = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->PCoML = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->Ptilde = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->Ptmp = gsl_matrix_calloc (nDoF-6,nDoF-6);
// [Allocated in ws]             ws->invJ = gsl_matrix_calloc (nDoF-6,6);
// [Allocated in ws]             ws->invJCoM = gsl_matrix_calloc (nDoF-6,3);
// [Allocated in ws]             ws->dq = gsl_vector_calloc(nDoF-6);
// [Allocated in ws]             ws->dqtmp = gsl_vector_calloc(nDoF-6);
// [Allocated in ws]             ws->dqtmp2 = gsl_vector_calloc(nDoF-6);
// [Allocated in ws]             ws->dq_old = gsl_vector_calloc(nDoF-6);
// [Allocated in ws]             ws->ddq_task = gsl_vector_calloc(nDoF-6);

// [Allocated in ws]             ws->opd = calloc(9,sizeof(double));
// [Allocated in ws]             ws->trace = gsl_vector_calloc (3);
// [Allocated in ws]             ws->adphi = gsl_vector_calloc(nDoF-6);
// [Allocated in ws]             ws->qdev = calloc(nDoF-6,sizeof(double));
// [Allocated in ws]             ws->CoP = gsl_vector_calloc (3);
// [Allocated in ws]             ws->zmp = gsl_vector_calloc (3);
// [Allocated in ws]             ws->dzmp = gsl_vector_calloc (3);

// [Allocated in ws]             ws->q_pd = gsl_vector_calloc(nDoF-6);
...
// [Allocated in ws]             ws->Jf = calloc(6*(nDoF-6),sizeof(double));
// [Allocated in ws]             ws->pf = calloc(3,sizeof(double));
// [Allocated in ws]             ws->Rf = calloc(9,sizeof(double));
// [Allocated in ws]             ws->taskf = calloc(6,sizeof(double));
// [Allocated in ws]             ws->invf = calloc((nDoF-6)*6,sizeof(double));
// [Allocated in ws]             ws->dqf = calloc((nDoF-6),sizeof(double));
        }
...
//MatPrintf( "ws->Jf", "%4.6f " , ws->Jf , 6, (nDoF-6)) ;
//printf(" \n \n");
...
//        gsl_blas_dgemv(CblasNoTrans, 1.0, ws->R, ws->p_task, 0.0, taskCoM);
        ...
        if (Visualisation)
        {
            FILE *p_file=fopen("./../Simu_data/ws->p_task.txt","a");
            for(n=0; n<3; n++)
            {
                fprintf(p_file,"%ws->f ",gsl_vector_get(ws->p_task,n));
            }
            fprintf(p_file,"\n");
            fclose(p_file);

            FILE *CoM_file=fopen("./../Simu_data/ws->CoM.txt","a");
            for(n=0; n<3; n++)
            {
                fprintf(CoM_file,"%ws->f ",gsl_vector_get(ws->CoM,n));
            }
            fprintf(CoM_file,"\n");
            fclose(CoM_file);
        }
...
//        if (Visualisation)
//        {
//            FILE *CoP_file=fopen("./../Simu_data/ws->CoP.txt","a");
//            for(n=0; n<3; n++)
//            {
//                fprintf(CoP_file,"%ws->f ",gsl_vector_get(ws->CoP,n));
//            }
//            fprintf(CoP_file,"\n");
//            fclose(CoP_file);
//        }
        ...
        if (Visualisation)
        {
            ...
            for(n=0; n<3; n++)
            {
                fprintf(taskCoM_file,"%ws->f ",gsl_vector_get(ws->taskCoML,n));
            }
...
// [Refactored to ws]         static double ws->wO=5.0;
// [Refactored to ws]         static double ws->amp=0.043;
...
//MatPrintf( "ws->taskf", "%4.6f " , ws->taskf , 6, 1) ;
//printf(" \n \n");
...
//gsl_vector_scale(ws->taskCoML,0.5);
        ...
        if (Visualisation)
        {
            ...
            for(n=0; n<3; n++)
            {
                fprintf(CoMp_file,"%ws->f ",gsl_vector_get(ws->p_task,n));
            }
            fprintf(CoMp_file,"\n");
            fclose(CoMp_file);
            ...
            FILE *taskCoMR_file=fopen("./../Simu_data/ws->taskCoMR.txt","a");
            for(n=0; n<3; n++)
            {
                fprintf(taskCoMR_file,"%ws->f ",gsl_vector_get(ws->taskCoMR,n));
            }
            fprintf(taskCoMR_file,"\n");
            fclose(taskCoMR_file);

            FILE *taskCoML_file=fopen("./../Simu_data/ws->taskCoML.txt","a");
            for(n=0; n<3; n++)
            {
                fprintf(taskCoML_file,"%ws->f ",gsl_vector_get(ws->taskCoML,n));
            }
...
//MatPrintf( "ws->invf", "%4.6f " , ws->invf , (nDoF-6), 6) ;
////MatPrintf( "ws->invf", "%4.6f " , ws->invf , 6, (nDoF-6)) ;
//printf(" \n \n");
//PrintGSLMatrix(ws->invJ);


//MatPrintf( "ws->dqf", "%4.6f " , ws->dqf , 1, (nDoF-6)) ;
//printf(" \n \n");
...
//PrintGSLVector(ws->dq);
        ...
        if (Visualisation)
        {
            ...
            FILE *u_file=fopen("./../Simu_data/ws->u.txt","a");
            for(n=0; n<nDoF-6; n++)
            {
                fprintf(u_file,"%ws->f ",uLINK[n+2].u_joint);
            }
            fprintf(u_file,"\n");
            fclose(u_file);
...
//    gsl_vector * ws->ddq_task = gsl_vector_alloc (nDoF);
//    gsl_permutation * ws->p_task = gsl_permutation_alloc (nDoF);
