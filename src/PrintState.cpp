...
void SaveStateXML(SuLINK uLINK[], State *Status, double t)
{
    ...
    for (i = 1; i < ((Status->dof) - 6 + 2); i++)
    {
        ...
        fprintf(f, "\t\t<p>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].p(0), uLINK[i].p(1), uLINK[i].p(2));
        fprintf(f, "\t\t</p>\n");
        fprintf(f, "\t\t<v>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].v(0), uLINK[i].v(1), uLINK[i].v(2));
        fprintf(f, "\t\t</v>\n");
        fprintf(f, "\t\t<vo>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].vo(0), uLINK[i].vo(1), uLINK[i].vo(2));
        fprintf(f, "\t\t</vo>\n");
        fprintf(f, "\t\t<w>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].w(0), uLINK[i].w(1), uLINK[i].w(2));
        fprintf(f, "\t\t</w>\n");
        fprintf(f, "\t\t<dvo>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].dvo(0), uLINK[i].dvo(1), uLINK[i].dvo(2));
        fprintf(f, "\t\t</dvo>\n");
        fprintf(f, "\t\t<dw>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].dw(0), uLINK[i].dw(1), uLINK[i].dw(2));
        fprintf(f, "\t\t</dw>\n");
        fprintf(f, "\t\t<hw>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].hw(0), uLINK[i].hw(1), uLINK[i].hw(2));
        fprintf(f, "\t\t</hw>\n");
        fprintf(f, "\t\t<hv>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].hv(0), uLINK[i].hv(1), uLINK[i].hv(2));
        fprintf(f, "\t\t</hv>\n");

        fprintf(f, "\t\t<R>\n");
        fprintf(f, "\t\t\t%f %f %f, %f %f %f, %f %f %f\n", uLINK[i].R(0, 0), uLINK[i].R(0, 1), uLINK[i].R(0, 2),
                                     uLINK[i].R(1, 0), uLINK[i].R(1, 1), uLINK[i].R(1, 2),
                                     uLINK[i].R(2, 0), uLINK[i].R(2, 1), uLINK[i].R(2, 2));
        fprintf(f, "\t\t</R>\n");
...
void SaveRobotXML(SuLINK uLINK[], State *Status)
{
    ...
    for (i = 1; i < (Status->dof - 4); i++)
    {
        ...
        fprintf(f, "\t\t<b>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].b(0), uLINK[i].b(1), uLINK[i].b(2));
        fprintf(f, "\t\t</b>\n");
        ...
        fprintf(f, "\t\t<c>\n");
        fprintf(f, "\t\t\t%f %f %f\n", uLINK[i].c(0), uLINK[i].c(1), uLINK[i].c(2));
        fprintf(f, "\t\t</c>\n");
        fprintf(f, "\t\t<R>\n");
        fprintf(f, "\t\t\t%f %f %f %f %f %f %f %f %f\n", uLINK[i].R(0, 0), uLINK[i].R(0, 1), uLINK[i].R(0, 2),
                                     uLINK[i].R(1, 0), uLINK[i].R(1, 1), uLINK[i].R(1, 2),
        ...
        fprintf(f, "\t\t<I>\n");
        fprintf(f, "\t\t\t%f %f %f %f %f %f %f %f %f\n", uLINK[i].I(0, 0), uLINK[i].I(0, 1), uLINK[i].I(0, 2),
                                     uLINK[i].I(1, 0), uLINK[i].I(1, 1), uLINK[i].I(1, 2),
                                     uLINK[i].I(2, 0), uLINK[i].I(2, 1), uLINK[i].I(2, 2));
        fprintf(f, "\t\t</I>\n");
...
void SaveStuctLink(SuLINK uLINK[], SaveLINK CopyuLINK[], State *Status)
{
    ...
    for (i = 1; i < ((Status->dof) - 6 + 2); i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            CopyuLINK[i].p[j] = uLINK[i].p(j);
            CopyuLINK[i].v[j] = uLINK[i].v(j);
            CopyuLINK[i].vo[j] = uLINK[i].vo(j);
            CopyuLINK[i].w[j] = uLINK[i].w(j);
            CopyuLINK[i].dvo[j] = uLINK[i].dvo(j);
            CopyuLINK[i].dw[j] = uLINK[i].dw(j);
            CopyuLINK[i].hw[j] = uLINK[i].hw(j);
            CopyuLINK[i].hv[j] = uLINK[i].hv(j);
            for (k = 0; k < 3; k++)
            {
                CopyuLINK[i].R[3 * j + k] = uLINK[i].R(j, k);
            }
        }
    }
}
...
void LoadStuctLink(SuLINK uLINK[], SaveLINK CopyuLINK[], State *Status)
{
    ...
    for (i = 1; i < ((Status->dof) - 6 + 2); i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            uLINK[i].p(j) = CopyuLINK[i].p[j];
            uLINK[i].v(j) = CopyuLINK[i].v[j];
            uLINK[i].vo(j) = CopyuLINK[i].vo[j];
            uLINK[i].w(j) = CopyuLINK[i].w[j];
            uLINK[i].dvo(j) = CopyuLINK[i].dvo[j];
            uLINK[i].dw(j) = CopyuLINK[i].dw[j];
            uLINK[i].hw(j) = CopyuLINK[i].hw[j];
            uLINK[i].hv(j) = CopyuLINK[i].hv[j];
            for (k = 0; k < 3; k++)
            {
                uLINK[i].R(j, k) = CopyuLINK[i].R[3 * j + k];
            }
        }
    }
}
