...
//        rot  = eye(3) + drot;
        ...
        gsl_blas_dger (1.0, wn, wn, tmp);
        gsl_blas_dgemv(CblasNoTrans, 1.0, tmp, vo, 0.0, tmp2);
        gsl_vector_scale ( tmp2, th);
        gsl_vector_add (uLINK[j].p, tmp2);

//        p2 = rot * uLINK(j).p -drot*cross(wn, vo) + wn * wn' * vo * th;
        gsl_blas_dgemm (CblasNoTrans, CblasNoTrans, 1.0, rot, uLINK[j].R, 0.0, tmp);
        gsl_matrix_memcpy (uLINK[j].R, tmp);
//             R2 = rot * uLINK(j).R;
//             end
        gsl_vector_free(wn);
        gsl_vector_free(vo);
